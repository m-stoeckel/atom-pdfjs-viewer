...
export default class PdfjsViewerView {
  ...
  // Call synctex for forward sync
  // compatible with thomasjo/atom-latex API
  forwardSync(texPath, lineNumber) {
    const command = atom.config.get('pdfjs-viewer.synctexPath');
    const cmd = `${command} view -i "${lineNumber}:0:${texPath}" -o "${this.pdfPathname}"`;
    ...
    cp.exec(cmd, (error, stdout) => {
      if (!error) {
        ...
        this.element.contentWindow.PDFViewerApplication.page = page;
      } else {
        console.error(error);
      }
    });
